...
<div class="stock-form-card">
    <div class="sf-header"><i class="fas fa-boxes"></i><strong>Ajouter un stock</strong></div>
    <div class="sf-body">
        @if ($errors->any())
            <div class="error-box"><ul style="margin:0;padding-left:18px;">@foreach($errors->all() as $e)<li>{{ $e }}</li>@endforeach</ul></div>
        @endif
        <form action="{{ route('admin.stocks.store') }}" method="POST" id="stockForm">
            @csrf
            <label for="warehouse_id">Magasin</label>
            <select name="warehouse_id" id="warehouse_id" required>
                <option value="">Sélectionner…</option>
                @foreach($warehouses as $w)
                    <option value="{{ $w->id }}" {{ ($preselectedWarehouse ?? '') == $w->id ? 'selected' : '' }}>{{ $w->name }}</option>
                @endforeach
            </select>
            ...
            <select name="stock_type_id" id="stock_type_id" required>
                ...
                @foreach($stockTypes as $t)
                    <option value="{{ $t->id }}">{{ $t->display_name }} ({{ $t->unit }})</option>
                @endforeach
            ...
            <input type="text" name="item_name" id="item_name" placeholder="Ex: Riz brisé, Huile 20L, Gasoil…" required>

            <div class="row2">
                <div>
                    <label for="quantity">Quantité initiale</label>
                    <input type="number" name="quantity" id="quantity" min="0" step="0.01" required>
                </div>
                <div>
                    <label for="min_quantity">Seuil minimum</label>
                    <input type="number" name="min_quantity" id="min_quantity" min="0" step="0.01">
                </div>
            </div>
            <div class="row2">
                <div>
                    <label for="max_quantity">Capacité max</label>
                    <input type="number" name="max_quantity" id="max_quantity" min="0" step="0.01">
                </div>
                <div>
