    ...
    <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
            ...
            <div>
                ...
                <select id="department" name="department" required class="form-input @error('department') error @enderror">
                    ...
                    <option value="Logistique" {{ old('department', $user->department) === 'Logistique' ? 'selected' : '' }}>Logistique</option>
                                            <option value="Sécurité Alimentaire et Résilience" {{ old('department', $user->department) === 'Sécurité Alimentaire et Résilience' ? 'selected' : '' }}>Sécurité Alimentaire et Résilience</option>
                    <option value="Résilience" {{ old('department', $user->department) === 'Résilience' ? 'selected' : '' }}>Résilience</option>
        ...
        <div style="margin-top: 1.5rem;">
            ...
            <textarea id="address" name="address" rows="3"
                      ...
                      placeholder="Adresse complète de l'agent">{{ old('address', $user->address) }}</textarea>
            @error('address')
                <p style="margin-top: 0.5rem; font-size: 0.75rem; color: #dc2626;">{{ $message }}</p>
            @enderror
        </div>
